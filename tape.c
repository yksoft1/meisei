...
static BOOL CALLBACK tape_blockrename_dialog(HWND dialog,UINT msg,WPARAM wParam,LPARAM lParam)
{
	switch (msg) {
...
BOOL CALLBACK tape_blockinfo_dialog(HWND dialog,UINT msg,WPARAM wParam,LPARAM lParam)
{
	switch (msg) {
		...
		case WM_RBUTTONUP: {
			...
			switch (id) {
				...
				case IDM_MEDIATAPE_RENAME:
					...
					DialogBox(MAIN->module,MAKEINTRESOURCE(IDD_MEDIATAPE_RENAME),dialog,tape_blockrename_dialog);
				...
				case IDM_MEDIATAPE_INFO:
					...
					DialogBox(MAIN->module,MAKEINTRESOURCE(IDD_MEDIATAPE_INFO),dialog,tape_blockinfo_dialog);
...
void tape_draw_listbox(HWND lb)
{
	...
	/* select */
	if (GetListBoxInfo(lb)>tape_od->cur_block) SendMessage(lb,LB_SETCURSEL,tape_od->cur_block,0);
