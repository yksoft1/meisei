...
void crystal_settings_load(void)
{
	...
	crystal->overclock=100;			if (SETTINGS_GET_INT(settings_info(SETTINGS_CPUSPEED),&i)) { CLAMP(i,OC_MIN,OC_MAX); crystal->overclock=i; }
	crystal->speed_percentage=300;	if (SETTINGS_GET_INT(settings_info(SETTINGS_SPEEDPERC),&i)) { CLAMP(i,101,SPEED_MAX); crystal->speed_percentage=i; }
	crystal->slow_percentage=30;	if (SETTINGS_GET_INT(settings_info(SETTINGS_SLOWPERC),&i)) { CLAMP(i,SLOW_MAX,99); crystal->slow_percentage=i; }
}
...
void crystal_set_cpuspeed(int p)
{
	...
	if (p!=crystal->overclock) {
		crystal->overclock=p;
		LOG(LOG_MISC|LOG_COLOUR(LC_GREEN)|LOG_TYPE(LT_CPUSPEED),"%d%% Z80 clock speed   ",p);
...
void crystal_set_mode(int show)
{
	...
	if (show&&pal!=crystal->mode) LOG(LOG_MISC|LOG_COLOUR(LC_GREEN)|LOG_TYPE(LT_VIDFORMAT),"%s video frequency ",pal?"50Hz":"60Hz");
	crystal->mode=pal;
...
BOOL CALLBACK crystal_timing(HWND dialog,UINT msg,WPARAM wParam,LPARAM lParam)
{
	switch (msg) {
		...
		case WM_INITDIALOG: {
			...
			if (crystal_get_input_trigger_caption(ic,CONT_T_MAX+INPUT_T_SLOWDOWN)||crystal_get_input_trigger_caption(ic,CONT_T_MAX+INPUT_T_SPEEDUP)) {
				...
				if (strlen(ic)&&strlen(ic2)) strcat(ic2,", "); strcat(ic2,ic);
			...
			switch (wParam) {
				...
				case IDC_TIMING_REVERSE_SLIDER: {
					...
					if (pos) {
						...
						sprintf(c,"%d:%02d, %d MB",sec/60,sec%60,(200*reverse_get_state_size()+reverse_get_buffer_size_size(pos)*(reverse_get_state_size()+100*CONT_MAX_MOVIE_SIZE))>>20);
