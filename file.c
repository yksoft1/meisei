			...
			if (patch.bytes) {
				...
				/* normal read */
					...
					if (!c) { /* no return on error: patched result size may be larger than source size */
						...
						if (size>=patch.show_size) LOG(LOG_MISC,"e"); /* extend */
					}
					patch.offset+=o;
				}
			}
...
void file_init(void)
{
	...
	file->appname=file->name; file->name=NULL;
...
void file_directories(void)
{
	...
	#define FILE_GETDIR(x,y)												\
		...
		if (c&&strlen(c)&&stricmp(c,"<undefined>")) {						\
			...
			if (strlen(c)>dot) strcat(y,c+dot);								\
		}																	\
		MEM_CLEAN(c)
	...
	#define FILE_PUTDIR(x,y)											\
		if (y&&strlen(y)) {												\
			...
			if (strlen(y)>=strlen(file->appdir)) {						\
				...
				if ((stricmp(c,file->appdir)==0)&&((strlen(y)==strlen(file->appdir))||((y[strlen(file->appdir)]=='\\')|(y[strlen(file->appdir)]=='/')))) off=strlen(file->appdir); \
			}															\
			memset(c,0,STRING_SIZE);									\
			c[0]='.';													\
			if (off<strlen(y)) memcpy(c+(off!=0),y+off,strlen(y)-off);	\
...
void file_setfile(char** filedir,const char* filename,const char* fileext,const char* zipextension)
{
	if (filename&&strlen(filename)) {
		...
		if (len>=STRING_SIZE) { LOG(LOG_MISC|LOG_ERROR,"Filename too long!\n"); exit(1); }
			...
			#define FILEDIR_APP_B() if (filedir==&file->batterydir) FILEDIR_WARN("sram");	\
			...
			#define FILEDIR_APP_E() if (SetCurrentDirectory(file->appdir)==FALSE) { LOG(LOG_MISC|LOG_ERROR,"Can't locate application directory!\n"); exit(1); } \
...
static void file_browse_directory(HWND dialog,UINT edit_id)
{
	...
	sprintf(c,"Select %s directory:",ct);
...
BOOL CALLBACK file_directories_dialog(HWND dialog,UINT msg,WPARAM wParam,LPARAM lParam)
{
	switch (msg) {
