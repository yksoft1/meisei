	...
	for (;;) {
		...
		if (file_open()) {
			...
			if ((len>4)&&(stricmp((file->filename_in_zip_post+len-4),".wav")==0)) {
				...
				#define SAMPLE_SEEK(x) o+=(x); if (o>=file->size) { SAMPLE_WARN(2); }
				...
				if (!inv) {
					...
					if (file_read(sf,file->size)) {
						...
						if (chans==2) {
							l>>=1;
							for (i=0;i<l;i++) sb[i]=((s16)sb[i<<1]+(s16)sb[i<<1|1])/2;
						}
...
void sample_stream(_sample* sample,int slot,signed short* stream,int len)
{
	...
	if ((len!=0)&sample->playing&(sample->len[sample->cur]>0)&(sample->bc<sample->len[sample->cur])) {
		int i;
		
		for (i=0;(i<len)&(sample->bc<sample->len[sample->cur]);i++) { stream[i]+=sample->sam[sample->cur][sample->bc++]; }
		
		sample->playing=sample->bc!=sample->len[sample->cur];
