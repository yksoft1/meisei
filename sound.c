...
void sound_create_secondary_buffer(int c)
{
	...
	if (!c&&sound.buffer_secondary_size==cspf) return;
...
void sound_silence(int c)
{
	...
	for (i=0;i<(sound.buffer_size/sizeof(signed short));i++) sound.buffer[i]=sample;
...
void sound_write(void)
{
	...
	while (num_samples>0) {
		...
		for (;;) {
			...
			if ((samples-available_samples)>sound.sleeptime||sound.sleep_auto^sound.sleep) sound_sleep(1);
		}
