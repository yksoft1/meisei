...
/* cents offset dialog */
static BOOL CALLBACK psgtoy_centoffset_dialog(HWND dialog,UINT msg,WPARAM wParam,LPARAM lParam)
{
	switch (msg) {
...
/* main window */
BOOL CALLBACK psgtoy_window(HWND dialog,UINT msg,WPARAM wParam,LPARAM lParam)
{
	switch (msg) {
		...
		case WM_INITDIALOG: {
			...
			for (i=257;i<513;i++) psgtoy.piano.screen[i]=0x505060; psgtoy.piano.screen[513]=0x404040; /* 2nd line */
			...
			/* palette */
			#define PAL(x)											\
				psgtoy.wave.info.bmiColors[i].rgbRed=(x)>>16&0xff;	\
				psgtoy.wave.info.bmiColors[i].rgbGreen=(x)>>8&0xff;	\
				psgtoy.wave.info.bmiColors[i++].rgbBlue=(x)&0xff
			i=0;
			...
			switch (LOWORD(wParam)) {
				...
				case IDC_PSGTOY_VEDIT:
					if (psgtoy.wave.wnd&&!psgtoy.drawing&&psgtoy.enabled&&GetDlgItem(dialog,IDC_PSGTOY_VEDIT)) {
						...
						if (i>CUSTOM_AMP_MAX) { i=CUSTOM_AMP_MAX; SetDlgItemInt(dialog,IDC_PSGTOY_VEDIT,i,FALSE); }
						if (i!=custom_amplitude[cc]) custom_amplitude[cc]=i;
						
						/* set preset to custom if changed */
						if (custom_preset[cc]!=PSGTOY_PRESET_CUSTOM&&i!=custom_preset_amplitude[custom_preset[cc]]) {
		...
		case TOOL_REPAINT: {
			...
			for (i=0;i<3;i++) {
				...
				if (psgtoy.c_en[i]!=c_en[i]) {
					char t[0x10]; char tt[0x10]; char tn[0x10]; char te[0x10]={0,0};
		...
		case TOOL_POPUP: {
			...
			switch (id) {
				...
				case IDM_PSGTOYPIANO_CENT:
					...
					CLEAN_MENU();
					if (DialogBox(MAIN->module,MAKEINTRESOURCE(IDD_PSGTOY_CENT),dialog,psgtoy_centoffset_dialog)==1) {
