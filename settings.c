...
void settings_init(void)
{
	...
	if (!file_open()||((file->size<10)|(file->size>100000))) {
		...
		LOG(LOG_MISC|LOG_WARNING,"couldn't open settings\n"); file_close(); return;
	}
...
void settings_save(void)
{
	...
	if (!file_save_text()) { LOG(LOG_MISC|LOG_WARNING,"couldn't save settings\n"); file_close(); return; }
	...
	if (SETTINGS_PUT_STRING(NULL,c)<0) { LOG(LOG_MISC|LOG_WARNING,"couldn't write settings\n"); file_close(); return; }
				...
				if (line[off]!=' ') {
					...
					if (stricmp(comp,search)!=0) break;
						...
						/* get float */
							...
							in=0.0; sscanf(val,"%f",&in); inv=in==0.0;
							in=1.0; sscanf(val,"%f",&in); inv&=(in==1.0);
...
int settings_put(const int type,const char* set,const char* c,const int i,const float f)
{
	...
	if (set&&(strlen(set)>0)) {
		...
		if (strlen(set)>=al) al=strlen(set)+1;
