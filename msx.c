	...
	while (msx.is_running) {
		...
		if (msx.paused) {
			...
			while (msx.is_running&&msx.paused) {
				...
				if (!msx.wait_paused) {
					if (msx.paused&3) {
						LOG(LOG_MISC|LOG_COLOUR(LC_ORANGE)|LOG_TYPE(LT_PAUSE),msx.pauselog_visible?"= paused =":"          ");
						msx.paused&=3;
					}
...
static int __fastcall msx_state_load_h(int v,u8** s,_msx_state* state,char* conflict)
{
	...
	if (j!=mapper_get_ramslot()) { sprintf(conflict,"expected RAM in slot %d",j); return FALSE; }
