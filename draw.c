...
static const char* draw_vidsignal_name[DRAW_VIDSIGNAL_MAX]={
	"RGB",				"Monochrome",			"Composite",		"S-Video",
	"custom"
};
static const char* draw_decoder_name[DRAW_DECODER_MAX]={
	"Industrial",		"Sony CXA2025AS/U",		"Sony CXA2025AS/J",	"Sony CXA2095S/U",
	"Sony CXA2095S/J",	"custom"
};
...
typedef IDirect3D9* (WINAPI *_D3DCREATE)(UINT);
...
void draw_create_text_mutex(void) { if ((draw.text_mutex=(CreateMutex(NULL,FALSE,NULL)))==NULL) { LOG(LOG_MISC|LOG_ERROR,"Can't create draw text mutex!\n"); exit(1); } }
...
static void draw_switch_renderer(void)
{
	...
	if (draw.rendertype==DRAW_RENDERTYPE_D3D&&err!=0) {
		/* d3d error, try ddraw */
		(*drawdev_clean)();
		LOG(LOG_MISC|LOG_WARNING,"D3D error %d, switching to DirectDraw\n",err);
...
int draw_init(void)
{
	...
	if ((draw.sc_mutex=(CreateMutex(NULL,FALSE,NULL)))==NULL) { LOG(LOG_MISC|LOG_ERROR,"Can't create draw sc mutex!\n"); exit(1); }
	if ((draw.d_mutex=(CreateMutex(NULL,FALSE,NULL)))==NULL) { LOG(LOG_MISC|LOG_ERROR,"Can't create draw d mutex!\n"); exit(1); }
	...
	LOG(LOG_VERBOSE,"draw initialised\n");
...
static float draw_changeedit_float(HWND dialog,u32 edit,float v,float min,float max)
{
	...
	if (GetDlgItemText(dialog,edit,(LPTSTR)f,0x10)) {
		in=0.0; sscanf(f,"%f",&in); inv=in==0.0;
		in=1.0; sscanf(f,"%f",&in); inv&=(in==1.0);
	}
...
/* custom video signal */
BOOL CALLBACK draw_change_vidsignal_custom(HWND dialog,UINT msg,WPARAM wParam,LPARAM lParam)
{
...
/* palette */
/* palette edit */
static BOOL CALLBACK draw_palette_edit(HWND dialog,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	switch (msg) {
		...
		case WM_CTLCOLORSTATIC: {
			...
			if (i>=IDC_PALEDIT_00&&i<=IDC_PALEDIT_15) {
				...
				return (BOOL)draw.paledit_brush[i];
			}
			break;
		}
			...
			switch (wParam) {
				...
				case IDC_PALEDIT_RSLIDER: case IDC_PALEDIT_GSLIDER: case IDC_PALEDIT_BSLIDER:
					if (initdone&&GetDlgItem(dialog,current)!=NULL&&GetDlgItem(dialog,wParam)!=NULL) {
						...
						if (i!=j) SetDlgItemInt(dialog,IDC_PALEDIT_REDIT+m,j,FALSE);
					}
					break;
			...
			switch (LOWORD(wParam)) {
				...
				case IDC_PALEDIT_REDIT: case IDC_PALEDIT_GEDIT: case IDC_PALEDIT_BEDIT:
					if (initdone&&GetDlgItem(dialog,current)!=NULL&&GetDlgItem(dialog,LOWORD(wParam))!=NULL) {
						...
						if (j!=i) SendDlgItemMessage(dialog,IDC_PALEDIT_RSLIDER+m,TBM_SETPOS,TRUE,(LPARAM)(LONG)i);
					}
					break;
...
/* palette main */
BOOL CALLBACK draw_palette_settings(HWND dialog,UINT msg,WPARAM wParam,LPARAM lParam)
{
		...
		case WM_INITDIALOG: {
			...
			for (i=0;i<DRAW_DECODER_MAX;i++) {
				if (i==DRAW_DECODER_DEFAULT) {
					sprintf(t,"%s (Default)",draw_get_decoder_name(i));
					SendMessage(c,CB_ADDSTRING,0,(LPARAM)((LPCTSTR)t));
				}
			...
			switch (LOWORD(wParam)) {
				...
				#define DA_CHANGEEDIT(n,s)											\
					if (HIWORD(wParam)==EN_KILLFOCUS) {								\
						...
						if (i>359) i=359; draw_ntsc.decoder[n]=(float)i;			\
				...
				case IDC_PALGEN_EDIT:
					if (DialogBox(MAIN->module,MAKEINTRESOURCE(IDD_PALETTE_EDIT),dialog,draw_palette_edit)==1) {
...
static void draw_text_out(void)
{
	...
	for (yd=draw.text_scroll;;yd+=12) {
		...
		if (start) {
			...
			if (((DWORD)(ticks-dt->ticks))>=draw.text_max_ticks||dt->type&LT_IGNORE) {
...
void draw_text_add(int type,int colour,const char* text)
{
	...
	for (;;) {
		...
		if ((copy>0)&((len_dest+copy)<(sizeof(draw_text_cursor->text)-1))) memcpy(&draw_text_cursor->text[len_dest],&buffer[offset],copy);
...
static int d3d_init(void)
{
	...
	*/
	...
	if ((create=(_D3DCREATE)GetProcAddress(d3d.dll,"Direct3DCreate9"))==NULL) return 2;
	if ((d3d.d3d=(*create)(D3D_SDK_VERSION))==NULL) return 3;
	...
	if (dynamic_err) { draw.softrender=TRUE; LOG(LOG_MISC|LOG_WARNING,"D3D dynamic textures unsupported\n"); }
...
static void d3d_reset(void)
{
	...
	if (result!=D3D_OK) {
		...
		LOG(LOG_MISC|LOG_ERROR,"Can't reset Direct3D device!\n");
		draw_clean(); exit(1);
	}
}
...
static void d3d_vsync(void)
{
	if (msx_is_running()&&((draw.frameskip_set>0)|((draw.frameskip_set<1)&(sound_get_lostframes()==0)))&&draw.vsync&&(d3d.caps.Caps&D3DCAPS_READ_SCANLINE)&&MAIN->foreground) {
		...
		for (;;) {
			...
			if (sc_old<sc_hitc&&sc>=sc_hitc) break; /* ok */
...
static void d3d_blit(void)
{
	...
	if (rectchanged|d3d.vb_recalc_needed) {
		...
		if (IDirect3DVertexBuffer9_Lock(d3d.vb,0,0,&vb,0)==D3D_OK) {
			...
			LOG(LOG_MISC|LOG_ERROR,"Can't lock vertex buffer!\n");
			draw_clean(); exit(1);
		}
...
static int ddraw_lock(LPDIRECTDRAWSURFACE* surface,DDSURFACEDESC* desc)
{
	...
	if (result!=DD_OK) { LOG(LOG_MISC|LOG_WARNING|LOG_TYPE(LT_DDRAWLOCKFAIL),"DirectDraw lock failed"); return FALSE; }
	else return TRUE;
}
...
static void ddraw_flush_backsurface(void)
{
	...
	fx.DUMMYUNIONNAMEN(5).dwFillColor=0;
...
static void ddraw_flush(void)
{
	...
	fx.DUMMYUNIONNAMEN(5).dwFillColor=ddraw.desc_back.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount==16?PAL_NTSC16(bc):PAL_NTSC32(bc);
...
static HRESULT ddraw_create_surface(LPDIRECTDRAWSURFACE* surface,DDSURFACEDESC* desc,int flags,int width,int height)
{
	...
	if ((result=IDirectDraw_CreateSurface(ddraw.ddraw,desc,surface,NULL))!=DD_OK) {
		...
		LOG(LOG_MISC|LOG_WARNING,"DirectDraw failed to init HW rendering\n");
	...
	if ((result==DD_OK)&&((IDirectDrawSurface_GetSurfaceDesc(*surface,desc))!=DD_OK)) {
		LOG(LOG_MISC|LOG_ERROR,"Can't get DirectDraw surface description!\n");
		draw_clean(); exit(1);
	}
...
static int ddraw_recreate_screen(void)
{
	...
	DDI_TRY(ddraw_create_surface(&ddraw.surface_back,&ddraw.desc_back,DDS_BACK,draw.surface_width,draw.surface_height),"Can't create DirectDraw back surface!\n")
	draw_recreate_font();
	draw_set_repaint(2);
...
static int ddraw_init(void)
{
	...
	DDI_TRY(DirectDrawCreate(NULL,&ddraw.ddraw,NULL),"Can't create DirectDraw!\n")
	ddraw.caps.dwSize=sizeof(DDCAPS); DDI_TRY(IDirectDraw_GetCaps(ddraw.ddraw,&ddraw.caps,NULL),"Can't get DirectDraw capabilities!\n")
	DDI_TRY(IDirectDraw_SetCooperativeLevel(ddraw.ddraw,MAIN->window,DDSCL_NORMAL),"Can't set DirectDraw cooperative level!\n")
	DDI_TRY(ddraw_create_surface(&ddraw.surface_primary,&ddraw.desc_primary,DDS_PRIMARY,0,0),"Can't create DirectDraw primary surface!\n")
	DDI_TRY(IDirectDraw_CreateClipper(ddraw.ddraw,0,&ddraw.clipper,NULL),"Can't create DirectDraw clipper!\n")
	DDI_TRY(IDirectDrawClipper_SetHWnd(ddraw.clipper,0,MAIN->window),"Can't set DirectDraw clipper to window!\n")
	DDI_TRY(IDirectDrawSurface_SetClipper(ddraw.surface_primary,ddraw.clipper),"Can't attach DirectDraw clipper to primary surface!\n")
	DDI_TRY(ddraw_create_surface(&ddraw.surface_back,&ddraw.desc_back,DDS_BACK,draw.surface_width,draw.surface_height),"Can't create DirectDraw back surface!\n")
...
static void ddraw_render(void)
{
	...
	ti_ntsc_blit_##x(														\
		...
		ddraw.desc_back.DUMMYUNIONNAMEN(1).lPitch,							\
	...
	int i,j,k=0;															\
	u8* s; y* o;															\
	int max=draw.surface_height*ddraw.desc_back.DUMMYUNIONNAMEN(1).lPitch;	\
	for (i=0;i<max;i+=ddraw.desc_back.DUMMYUNIONNAMEN(1).lPitch,k++) {		\
	...
	switch (ddraw.desc_back.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount) {
		case 32:
		...
		default:
			...
			LOG(LOG_MISC|LOG_ERROR,"%d-bit depth is unsupported!\nswitch to 16 or 32\n",ddraw.desc_back.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount); exit(1);
			break;
	}
	...
	if (msx_is_running()&&((draw.frameskip_set>0)|((draw.frameskip_set<1)&(sound_get_lostframes()==0)))&&draw.vsync&&MAIN->foreground) {
		if (ddraw.caps.dwCaps&DDCAPS_READSCANLINE) {
			...
			for (;;) {
				...
				if (result!=DD_OK||sc>=height||(sc_old>(height-(height>>2))&&sc<(height>>2))) break;
			...
			if (!vblank&&sound_get_sleeptime_raw()==~0) { IDirectDraw_WaitForVerticalBlank(ddraw.ddraw,DDWAITVB_BLOCKBEGIN,0); }
		}
	}
}
