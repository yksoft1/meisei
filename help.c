...
/* help window */
BOOL CALLBACK help_help(HWND dialog,UINT msg,WPARAM wParam,LPARAM lParam)
{
	switch (msg) {
		...
		case WM_INITDIALOG: {
			...
			/* open documentation */
			file_setfile(&file->appdir,"meisei.txt",NULL,NULL);
...
BOOL CALLBACK help_about(HWND dialog,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	switch (msg) {
		...
		case WM_LBUTTONDOWN: {
			...
			if (cursor_in_dialogitem(dialog,IDC_ABOUT_URL)) {
				if (((UINT)ShellExecute(NULL,"open",VERSION_URL_S,NULL,NULL,SW_SHOWNORMAL))>32) { url_clicked=TRUE; action=2; }
				else action=1;
			}
			...
			/* open new email */
				...
				if (((UINT)ShellExecute(NULL,"open",c,NULL,NULL,SW_SHOWNORMAL))>32) { email_clicked=TRUE; action=2; }
				else action=1;
			}
		...
		case WM_COMMAND:
			...
			switch (LOWORD(wParam)) {
				...
				/* close dialog */
				case IDOK: case IDCANCEL:
					if (underline!=NULL) DeleteObject(underline); underline=NULL;
					if (icon!=NULL) DestroyIcon(icon); icon=NULL;
					EndDialog(dialog,0);
					break;
